...
deterministic benchmarks on a variety of well known VMs. Our results
shown that less than one quarter of cases \edd{careful. That statistic applies
to a quarter of VM/benchmark pairings, not individual process executions, which
the reader could mistake this for. Since we haven't described our method (and
what such a pairing is) yet, I would use softer terminology like "few"?}
conform to
the traditional notion of warmup, and that none of the
...
into a list\edd{It prints the result immediately to avoid in-process memory
consumption of the iterations runner from increasing over time. Not sure how to
word this though.}; when all iterations are complete, it returns the results
