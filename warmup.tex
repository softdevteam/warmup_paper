...
\newcommand{\bencherthree}{Linux$_\textrm{4790K}$\xspace}
\newcommand{\bencherfive}{Linux$_\textrm{4790}$\xspace}
\newcommand{\benchersix}{OpenBSD$_\textrm{4790}$\xspace}
\newcommand{\bencherseven}{Linux$_\textrm{E3-1240v5}$\xspace}
...
cycle count deltas and APERF/MPERF ratios\edd{reference to core-cycles and
ratios before this terminology is introduced}.
...
throttled\edd{can we find a less informal term that "throttled"}; and above 1, turbo boost is being used.
...
driver to allow us to access the MSRs as a non-root user\edd{Not quite right.
We can access the MSRs as a non-root user with the standard msr driver if we:
tag the binaries with the right capabiliby flag (annoyong) and avoid use of
LD\_LIBRARY\_PATH, which krun uses a lot, but which is illegal for capability
tagged binaries. Hard to explain in one sentence...}} which are relatively
...
integer values are automatically promoted to floating point upon exceeding their maximum.}
...
(in similar fashion to \kalibera). 4 benchmarks showed what we have come to
...
an open research challenge in changepoint analysis. Since this only happens for 2
of 47 benchmarks \edd{what is a benchmark here? s/benchmarks/process executions/?}, both of which have a small absolute difference between the
...
time). Figure~\ref{fig:startup} shows the data for two
of the benchmarking machines. As this data clearly shows, there is substantial
variation in the startup time of various VMs.
\edd{Since we reboot after each run, arguably a large chunk of the times is
I/O. Maybe for the real run we should not reboot and consider the startup times
themselves to "warmup" due to I/O caching}

\begin{figure}[ht]
\centering
\input{startup.table}
\caption{Startup times for the VMs.}
\label{fig:startup}
\end{figure}
