...
performers at the point of that publication. We noted that newer (and possibly
faster) implementations of the CLBG benchmarks are now available, however we
did not use them.
...
this effect.\edd{What happens when a VM forks. Does the child stay on the
...
\texttt{clock\_gettime} function as standard. V8, HHVM and JRuby/Truffle were
patched so as to expose the monotonic clock via a user-visible function. On
OpenBSD, we use
