...
characteristics are statistically equivalent. Checking for equivalence involves
checking whether means and confidence intervals overlap \edd{I'm not sure you
can say two means overlap. That would be straight equality. Do you mean that
the interval for a measure of the mean overlaps?}\laurie{what i mean
is `the area either side of the mean covered by the CI'. i don't know if
there's a nice name for that.}.\footnote{Note that there is an unavoidable
...
no steady state process executions must be handled separately.
...
steady perf (s) as either succeeding or failing (since no comparison can be made).
...
samples (using sampling with replacement) of size $n$.  One unintuitive side effect of sampling is that
