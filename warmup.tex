...
After running our experiment, we have \laurie{how many?} data-points to examine,
which we present as graphs. For each graph, we show: the benchmark name, the
name of the machine on which it was executed, and its process execution number;
in-process executions, in order, on the $x$-axis; and the time per in-process
execution on the $y$-axis. These graphs, though simple, highlight a number of
...
when classifying data. Figure~\ref{fig:examples:outliers1} shows a typical
...
\caption{A benchmark with an outlier (at iteration \laurie{be precise about which iteration number the outlier is})..}
...
\caption{Two examples of well-behaved, traditional, warmup behaviour.\laurie{for this version of the paper, let's not mess with the x-axis scale---it's just something else to confuse people.}}
\label{fig:examples:trad}
\end{figure*}

\textbf{Traditional Warmup}
Of the \laurie{XXX} benchmark / VM pairs, \laurie{YYY} consistently warmed-up as
...
\caption{Example of a benchmark with a slowdown occurring at in-process
iteration \laurie{be precise about the number}.}
...
drift over many in-process executions \laurie{let's put a graph like that in!}.
...
\caption{A benchmark with cycles \laurie{``of length XXX'' and maybe ``starting at in-process iteration YYY''?}}
...
we say `from XXX to YYY', to give people an idea of the scale?}: the example in
Figure~\ref{fig:examples:cycles} shows an example with a cycle of 7 \laurie{this
is clearly wrong: if it was 7, there would be 285 spikes in that figure}
...
\caption{A benchmark with a late phase change (in this case entering slowdown
at in-process iteration \laurie{XXX}).}
...
machines.\cfbolz{I know that it would be slightly annoying to do, but can we have the same axes for the same benchmarks here?}\laurie{agreed: we really need to do that}}
...
`warmup' styles and note the relatively high frequency of `unexpected'
