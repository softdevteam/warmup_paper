...
processes. We expected this to validate the above
...
performance variations\edd{I removed a citation about geometric means from
here(\cite{fleming86notlying}). Did we mean to cite something else?}, however,
...
performance}\edd{Maybe I know too much from seeing our graphs, but theres
always the opportunity for future compilation due to a program taking an
as-of-yet untrodden code-path. I don't like to think that there is a moment in
time after which no compilation will ever occur, although we seem to imply
this. Maybe we should delay the definition of warmup until after the graph and
explain that this is how this mystical notion is said to work}.

\begin{figure}[h!]
\centering
\includegraphics[width=.4\textwidth]{img/trad}
\caption{\laurie{the problem here is that we say `fictional', but the data looks vaguely real. we'd do better to make an obviously-not-real data figure, so that users can't misinterpret it. the glowworm proposal has something along these lines in it already}\edd{let's drop in a version of that in the new graph style} This (fictional) chart illustrates the commonly accepted understanding
...
is spending too much time in a \texttt{perf} sampling routine. Whilst this may
be useful in a general setting, for benchmarking it may adjust benchmark
...
mitigate memory layout attacks. ASLR is not, however, practical for
benchmarking, as memory layout can wildly impact benchmark
...
necessarily for benchmarking, since (e.g.) always reallocating at
\texttt{realloc} sites could invalidate caches etc. \krun uses the
