...
to the analysis of in-process iterations. For each \vmbpair pair and each
$n \in \{2, 3, \ldots, 29, 30\}$ we created 1000 bootstrapped samples (using sampling with replacement)
of size $n$ (i.e.~each set contains $n$ process executions). \laurie{sarah:
should we be talking about the 1000 trials?}
We then report the proportion of sampled \vmbpair pairs which are statistically
equivalent to the original. One unintuitive side effect of this sampling when
...
Whenever a sample of 30 warmup process executions is created, then all three
