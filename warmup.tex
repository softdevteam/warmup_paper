...
as the Unix user `\texttt{krun}' with a fresh environment. When run in
`production' mode, \krun reboots the system before each process execution (including
...
\krun sets the CPU frequency to the highest value possible
...
each physical processor chip counts as a CPU.} 250 times (`ticks') a second to
decide whether to perform a context switch. Using a tickless kernel, a
configurable subset of CPUs can be put into `adaptive-tick' mode, where ticks
are only delivered if there is more than one runnable process scheduled. We
experimented with a tickless kernel compiled with the
\texttt{CONFIG\_NO\_HZ\_FULL\_ALL} option set: this places all CPUs apart from
the first into adaptive-tick mode.\footnote{It is not possible to put all CPUs
into adaptive-tick mode.} When we compared a subset of benchmarks on a tickless
vs.~a standard
kernel, we noticed a small reduction in jitter, although not enough for us to
conclusively say that the tickless kernel was the cause. However, since our
tickless configuration does not appear to have negative effects, we opted to
run the real experiment under this configuration.
