...
performance. We introduce a new automated approach to determining when warmup has completed -- based
on changepoint analysis -- which allows us to classify if a program
has reached a steady states and, if so, whether that represents peak
...
time series data collected before warmup is complete is discarded, therefore
...
\end{description}
In order to address this hypothesis, we had to develop a new approach to
automatically analysing large quantities of VM data. We deliberately chose
...
than expected by VM teams we have spoken to. Our results suggest
that much real-world VM benchmarking is likely to be partly or wholly
misleading. This has two real-world consequences. Most obviously, microbenchmarks similar to those
in this paper are often used in isolation to gauge the efficacy of VM optimisations;
our findings suggest that some such optimisations may be ineffective or even
deleterious. More subtly, it suggests that large programs which appear to have
fairly predictable performance may have significant sub-parts which are behaving
unpredictably, but which, in large numbers, tend to be lost in the noise. Put
another way, it may be that we are being misled as to the ultimate performance
`ceiling' of large programs due to the effects we observe on microbenchmarks
(i.e.~we may be unduly pessimistic about the possible performance of large
programs).
...
We ran our experiment on different machines and operating systems; whilst different
operating systems, in particular, make some difference to the results we observe,
we are able to rule them out as being a significant factor.
...
\caption{Two examples of run-sequence plots from our experiment, augmented
...
label at the top shows the benchmark (e.g.~\binarytrees), the VM (e.g.~HotSpot),
the machine (e.g.~\bencherseven), the process execution number (e.g.~5),
and the classification (e.g.~warmup). The `main'
plot at the bottom always shows the complete data (i.e.~all 2000
in-process iterations, and a $y$ axis which allows the timings for all those
in-process iterations to be seen). Because this often obscures the finer
...
in-process timing data is located; the inset plot allows us to examine a handful
...
are the vertical red lines, and denote a `shift' in the nature of the time series
data; changepoint segments are the horizontal red lines between changepoints; red circles are `outliers'
which the changepoint analysis discounts when finding changepoints. We use the changepoint
...
from `slow' to `fast' modes after JIT compilation). We then use the changepoint
segments that this analysis produces to automatically classify process executions as:
...
unexpected performance patterns that previous methodologies have conflated.
...
often fail to warmup.
    \item We present the first automated approach to analysing VM
time series data that can automatically determine if and when a steady state has been
reached.
...
\laurie{i think this is no longer true, right?} Finally, \krun disables Address Space Layout Randomisation (ASLR). While ASLR is
...
faster or slower). For example, for traditional warmup, we might expect to see a
...
tricky: simple heuristics suffer from both false positives and negatives; and
...
of each segment. \laurie{Figure XXX shows an example. Should we use Figure 1 or does that
cause too much backpage flipping?}\edd{I'm going to include changepoint means on all plots, so it's probably not worth pointing out examples}
...
mean is within $s_f$'s mean $\pm$ $s_f$'s variance or 0.001s (whichever
is greater), we consider the two equivalent. The reason for these two types of tolerance
...
Based on this, we can then classify a \vmbpair
pair as follows:
...
iterations (`steady iter (\#)') and the time in seconds (`steady iter (s)') to
reach the steady state, as well as the performance of the steady state itself in
seconds (`steady perf (s)').

Steady iter (\#) and steady iter (s) allow one to understand how long it takes
a benchmark to reach a steady state, and are made possible by our use of changepoint
analysis. Flat process executions by definition have a steady iter (\#) of 1 and
a steady iter (s) of 0; we elide these details for benchmarks
...
impossible. We therefore use Inter-Quartile Ranges (IQRs) to give an indication of the
spread of values, reporting the 5\% and 95\% percentiles \laurie{nearest or
linear or ...?}. Non-overlapping IQRs imply a meaningful difference in the steady iter (\#) or
(s) values of a benchmark run on two VMs; however, since the IQRs are often
...
Steady perf (s) roughly corresponds to the `normal' performance number reported
...
calculated via \laurie{XXX rounds of} bootstrapping. We use standard
bootstrapping, which assumes that the timings within each segment are
independent. Although timings within segments are not always independent (see
Section~\ref{sec:cyclic}), `large' degrees of independence lead to
non-equivalent segments, meaning that at worst we can only bootstrap data with
`small' degrees of independence. We are therefore fairly safe in bootstrapping
such data. However, since the values in different segments are clearly not
...
\begin{table*}[hptb]
\centering
% Fudge factor of 5 rather than 2 here to leave room for the caption.
\begin{adjustbox}{totalheight=\textheight-5\baselineskip}
\input{bencher7.table}
\end{adjustbox}
\caption{Benchmark results for \bencherseven. For processes which reach a steady
...
% v0.7 calculations: Excluding the empty benchmark we have:
...
\sarah{the next few paragraphs compile but need re-writing for the new data.}
...
and \sarah{was: \\bencherfivevmbenchpairsinconsistentpercentage\%} are inconsistent
...
950, where the VM migrates from (tickless) core 1 to (ticked) core 0.\edd{revise caption}}
\label{fig:examples:nosteadystate}
\edd{XXX add another figure here to save/utilise space}
\end{figure}
...
iterations. \edd{XXX this is 0.7 data}}
\label{fig:examples:slowdown1}
\end{minipage}
\hfill
\begin{minipage}[b]{0.49\textwidth}
\includegraphics[width=\textwidth]{examples/new_cyclic.pdf}
\caption{Cycles in wall-clock times and core-cycle counts.\edd{Lots of space here due to long caption on the left. Either write more here, or shorten caption on the left}}
...
\laurie{edd: we need to update this section}
...
cycles are not correlated with GC (of which there is none) or JIT compilation.\edd{XXX update for 0.8}}
\label{fig:goodcomp}
\end{figure}

\section{Startup Time}
\label{sec:startup}
\sarah{startup table is still v0.7}
...
Table~\ref{fig:startup} shows startup times from both \bencherseven and \benchersix. Since
