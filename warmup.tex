...
different process executions; and many exhibit cyclic behaviour. Of the seven
...
provide a repeatable way of determining when warmup has completed, does
not deal with process executions which warmup at different points,
nor does it deal satisfactorily with cyclic data. Because of this
...
Analysing \krun's configuration file is a relatively memory intensive task which
we do not want to occur before every benchmark is run (since it is likely to
e.g.~fragment memory). Therefore, when first run, \krun analyses the
configuration file and creates from it a simplified benchmark running schedule
which is written to disk. After each reboot, it simply reads the schedule
line-by-line for the next benchmark to run, runs it, and then updates the
schedule. Once the benchmark has run, \krun can then safely read the whole
configuration again since the machine will be rebooted before the next benchmark
is run.
...
In some cases where there is a behavioral change in wall-clock times, the
...
decreased) wallclock times being a result of the corresponding CPU core doing
...
another (as yet unkown) source of variation. We did not observe an
...
printed out wall-clock time). Figure~\laurie{XXX} shows the data for each of our
