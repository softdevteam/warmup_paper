...
``obvious'' changepoint has been discovered, it is not worth searching behind it
for further changepoints\edd{Not enough contextual discussion to know the
meaning of ``behind''. So are we searching left to right then? Behind would be
further to the left? Would it be enough to say it uses a heuristic to find an
approximation?}. Candidate segments are compared relative to their fit (defined
as the negative log-likelihood, which is based on a model of the normal
distribution with changing mean and variance) and penalty (described above).
\edd{Lots of terminology I don't understand here. Is it worth simplifying and
referring the reader to the PELT paper? As it stands, unless the reader has
read the other paper, the last couple of sentences make little sense.
Alternatively you could add more detail, explaining briefly: segment fit,
negative log likelihood, and the model.}.
...
reach the steady state, as well as the in-process iteration time in seconds for
the steady state (\emph{steady perf (s)}).
...
should be run \edd{do we mean, how many times they are repeated, not how
often?} to obtain reliable statistics. There are several approaches which
aim to dynamically discover how often benchmarks in non-JIT compiling VM
contexts need to be run in order to obtain reliable statistics (see~\cite{chen16robust}
...
they expect, adapting them to become warmup-aware may not be trivial.
...
of further executions needed. Given the unpredictable nature of inconsistent process
...
reached. The fully automated open-source changepoint analysis-based
\edd{changepoint analysis-based sounds clumsy, but I can't think what to
replace it with} approach presented in
this paper is one option for analysing each process execution individually.
...
are several benchmarks where different VMs achieve steady state
...
they are important to the majority\edd{can you back that up? (not saying it is
wrong)}. We thus hope that future VM experiments will
...
running benchmarks for a longer period than previous methodologies had
suggested. Traditional VM
...
non-deterministic bugs are likely, at best, to frustrate those benchmarking
them \edd{There's two separate issues here: a) bugs, b) non-determinism. These
have been mixed together and make it a little confusing. I don't want any bugs,
non-deterministic or otherwise. Presumably you referring to dacapo bugs?} and,
