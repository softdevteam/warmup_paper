...
\vmbpair pairs consistently reach a steady state of peak performance.\edd{Should this really be machine, bench, vm \emph{triples}? Otherwise we would have one summary figure for all machines, right? Presumably we have a summary range because the summaries are per-machine.}\laurie{this is implied by `across 3 machines' IMHO, and i fear that `triples' just sounds confusing at this point. We could be clearer perhaps by adding ``(depending on the machine)'' after the fractions?}
\edd{yeah maybe. Or how would you find `combo` instead of triple?}\laurie{new terminology -- vague terminology at that! -- will only make the situation worse IMHO.}
\end{abstract}
...
\edd{inconsistent machine naming: linux$_{4790}$ vs. linux$_{e3-1240v5}$. Either
remove e3- from 1240v5 or add i7- to 4790} \laurie{i'm not sure but i think xeon numbering is more prone to duplications. but of the two i'd prefer to remove e3}
...
\edd{sarah: are the macros in summary\_macros.tex up to date?}
...
\edd{Edd has not yet read below here in the results section.}
