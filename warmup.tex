...
effect of many external events can add up. We found the standard deviation
too small to use as a proxy for this cumulative effect, but the variance (which is the
standard deviation squared) suited our purposes (despite the change in units from s to s$^2$).
We therefore combine these two notions,
...
in-process iterations from $0 \ldots n$ where $n \in \{10, 20, \ldots, 1990\}$),
