...
users, it gives no insight into multi-threaded computations: we therefore also record \laurie{on linux only?}
...
GC, or of other processes interrupting benchmarks. We use the method described
...
Based on the results of changepoint analysis, we can then start to characterise
...
one another by a (possibly short) segment with a clearly distinct mean or variance.
To understand how `noise floor' affects our analysis, it
...
when comparing a segment $s_i$ against the final segment $s_n$: if $\textrm{mean}(s_i)$
is within $\textrm{mean}(s_n) \pm \textrm{max}(\textrm{variance}(s_n), 0.001s)$,
...
is the index of the last iteration in the segment --
...
def classify(segments):
  ...
  while i > -1:
    ...
    if cur_seg.mean + cur_seg.variance >= lower_bound and cur_seg.mean - cur_seg.variance <= upper_bound:
      continue
    elif cur_seg.end > len(segments) - STEADY_STATE_LEN:
      cls = "no steady state"
      break
...
nuanced view than IQRs can report we also provide thumbnail histograms, marking
the histogram bin containing the median value in red.
...
\sarah{I'm not sure why run-sequence is italicised here, a run-sequence plot is just a plot of time-series data and we have used the term before.}
...
examples. On Linux we also use \emph{core-cycle plots}, showing the
...
\caption{An example of inconsistent process executions for the same \mvmbtrip triple.\laurie{can we find ones that are more obviously inconsistent? a reader could reasonably say "well, they both have the same steady state" (unless they're eagle eyed, they might not notice that they hit the steady state at different points)}}
...
\laurie{eek: the two y axis labels of the two instrumented plots are different. one has "secs" one doesn't. can we homogenise?}\edd{it's because the time units in PyPy are unknown and abstract}}
\label{fig:goodcomp}
\end{minipage}
\hfill
\begin{minipage}[t]{0.485\textwidth}
...
it is only collected when \krun is explicitly set to `instrumentation mode'. For
example, in Figure~\ref{fig:examples:slowdown1}, there is a clear correlation
(though we cannot be entirely sure that it is a causation) between a JIT
compilation and a slowdown. Similarly, Figure~\ref{fig:goodcomp} shows an
example where slowdown could be attributed to garbage collection. Note however,
that in many cases neither JIT compilation, nor garbage collection, are
sufficient in explaining odd behaviours. One such case is shown in
Figure~\ref{fig:unexplained}.
...
the user to manually pick one part of the cycle for, which leads to a clear potential for
bias. As our approach automatically identifies changepoint segments, we take a
very different approach. `Large' cycles (i.e.~where the means of segments are
substantially different) lead to segments which are considered non-equivalent
and lead to a classification of no steady state. `Small' cycles (i.e.~where the
means of segments are close enough to be considered equivalent) are then
naturally handled by bootstrapping within, but not between, segments (see
