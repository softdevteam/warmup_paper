...
However, we needed to check that bootstrapping within segments was safe. Of
the \totalpexecs{} process executions from our experiment, \laurie{XXX}
reach a steady state, comprised of \laurie{XXX} segments. \laurie{XXX}\% of
these segments have a variance below 0.001s, at which point we consider any data
to be noise and not worth considering for dependence. For the remaining
segments, we ran a Durbin-Watson test statistic to determine if they were
autocorrelated. Even with very conservative bounds (1.6 and 3.4 for lower
and upper bounds respectively), only \laurie{XXX}\% of segments have statistically
significant autocorrelation. Because of this very low figure, we thus
feel safe in bootstrapping within segments.
...
to consider this example `no steady state'. \laurie{this example
is confusing: unless you're eagle eyed the saw tooth in the CC plot
is invisible. can we find one with clearer saw teeth?}}
...
\caption{\laurie{the plot doesn't have instrumentation} Slowdown at in-process iteration \#199, with GC and JIT measurements
...
\caption{\laurie{this doesn't have instrumentation either} PyPy benchmarks often showed a gradual `ramp up' over time. This
