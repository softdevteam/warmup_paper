...
\end{minipage}
\caption{Main experiment results. \textsc{Key}: flat=\flatc,
...
(results from the other machines can be seen in Appendix~\ref{app:mainresults}).
\laurie{we should summarise: X\% are flat, Y\% are warmup etc.} These results
clearly invalidate Hypothesis H1.
...
has one per core. Figures~\ref{fig:examples:trad}, \ref{fig:examples:slowdown1},
\ref{fig:examples:nosteadystate} show examples of traditional warmup, slowdown,
...
HHVM (process execution 4 of 10 run on \bencherthree). Warmup completed by
in-process iteration \#134. In this example, the main benchmark computation
occurred almost entirely on core 1: cores 0, 2 and 3 had Cycle Counts (CC) close
to 0 for the entire in-process execution and are thus not shown.}
...
Figure~\ref{fig:examples:cycles} shows a case where the VM migrates
mid-process
...
\centering
\input{dacapo.table}
\caption{Dacapo results. \textsc{Key}: flat=\flatc,
warmup=\warmup, slowdown=\slowdown, inconsistent=\inconsistent.}
\label{tab:dacapo}
\end{table}
...
iterations runners make, some still remain for taking measurements. We call
\texttt{clock\_gettime()} to take monotonic in-process iteration start and stop times.
In both the Linux and OpenBSD kernels, reading the monotonic clock is not
atomic. To ensure a correct clock value is obtained, there is an (unbounded)
read loop, which terminates only if the clock value did not change during the
read.~\footnote{See \texttt{binuptime} in \texttt{sys/kern/kern\_tc.c} in the
OpenBSD source code and \texttt{getrawmonotonic} in
\texttt{kernel/time/timekeeping.c} in the Linux source code.} In
...
figure on it}. Reading from a MSR device node requires filesystem access and
then inter-processor interrupts to
