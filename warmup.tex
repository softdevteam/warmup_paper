...
results. We perform comparisons and report the percentage of \vmbpair pairs
that are `similar' as shown in Table~\ref{tab:mainresults}. We report
individual percentages for the classifications, steady iter (\#, s),
and steady perf (s). We also report `overall similarity': the percentage of
pairings for which all three metrics remain the same.

All similarity comparisons involve checking if two confidence intervals overlap.
Consider a \vmbpair pair $P$ from the original results and a \vmbpair pair $P_s$
sampled from $P$. If the steady iter (s) confidence interval for $P$ overlaps
with that of $P_s$, then the steady iter (s) is `similar'. The same applies for
steady iter (\#) and steady perf (s).~\footnote{Note
...
To make comparisons of classifications we constructed multinomial proportion
confidence intervals (i.e.~the proportion of warmup, slowdown, flat and no
steady state classifications) on the original set and each sampled set using
the method of \cite{sison95simultaneous}.

There are a noteworthy corner-cases relating to the no steady state classifier.
\edd{untouched chunk, would need to say about things being `considered similar', not `comparing true'}.
