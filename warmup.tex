...
and the automatic classification (e.g.~warmup). The `main'
plot at the bottom always shows the in-process iteration number (indexed from 1) versus the corresponding wallclock times for all 2000
in-process iterations.
Because this often obscures the finer
...
are indicated by vertical red lines and denote a `shift' performance behaviour;
...
often fail to warmup.
    \item We present the first automated approach to analysing benchmarking
...
\vspace{-.5em}
\edd{urls are usually in typewriter, not sure why this one is not.}
\begin{center}
