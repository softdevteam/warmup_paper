...
as many process executions and as many in-process iterations as we did. In this
subsection we sample smaller quantities of both to see if doing so would change our
overall results.
...
eventual choice of \numpexecs was slightly arbitrary. To understand if we could
have used fewer process executions, we have to use a slightly different method
...
classifications between sets, we constructed multinomial proportion confidence
intervals (roughly speaking, the proportion of warmup, slowdown, flat and no
steady state classifications) on the original set and each sampled set using the
method of \cite{sison95simultaneous}.
We then compared the resulting
classification analysis, times to reach a steady state, and steady state
performances to the original ($n=30$). As for the truncated in-process
iterations we also computed an `overall' similarity score taking into account
all three characteristics. Figure \ref{fig:pexecstruncated} shows the
