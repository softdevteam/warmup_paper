...
approach requires $\mathcal{O}(2^(n-1))$ calculations ($2^1999$ in our context).
More sophisticated changepoint analysis algorithms reduce this to a more
manageable, but still rather large, $\mathcal{O}(n^3)$. We utilise the PELT
algorithm~\cite{killick12optimal} which reduces the problem by noting that once
an ``obvious'' changepoint has been discovered, it is not worth searching behind
it for further changepoints. This reduces the complexity to $\mathcal{O}(n^2)$
...
reached. The fully automated open-source changepoint analysis-based approach presented in
