...
vary its environment (we recorded and verified this for all process
executions), and we designed the experiment such that, for a particular (machine, VM,
benchmark) triple the additional environment used to configure the VM is constant
size.
...
\caption{An example of changepoint analysis, from execution \#0 of Fasta, Hotspot
on \benchersix. The changepoint analysis has detected two changepoints,
one soon after the benchmark started, and one just before iteration 600. An
outlier (which is ignored by the changepoint software) is circled in red.
The segment means are shown as horizontal red lines, and show that an early improvement
in the benchmark performance was followed by a slowdown of approximately 0.03s.}
\label{fig:examples:changepoint}
\vspace{-.2cm}
\end{figure}
...
we classify the pair the same (in this example, warmup); otherwise we classify the pair as
...
\benchersix are broadly similar. Taking \bencherthree is an example,
one third of its (VM, benchmark) pairs are `bad', leading us to clearly reject
Hypothesis H1.
...
\sarah{The below may need moving around.} Columns describing when a benchmark
pair has reached a steady state, how long the VM took to reach a steady state
and mean performance thereafter has been aggregated over 10 process executions.
In each case a median is given along with confidence interval
(bootstrapped with 10000 iterations, by sampling with replacement).
...
allow us to observe that the slowdown is preceeded by two JIT compilation events
that are likely to explain the drop in performance. Note also that the
...
\begin{description}
  \item[H3] Non-warmup process executions are due to JIT compilation or GC events.
\end{description}
...
\laurie{i suggest we duplicate figure 7 (the cycles) which a) shows that
instrumentation \emph{does} change the run-sequence plot a bit, but also that
that particularly odd example isn't explained by GC or JIT compilation}

Our results leads us to tentatively invalidate Hypothesis H3: whilst some
non-warmups are explained by GC or JIT compilation events, many are not, at
least on HotSpot and PyPy. In such cases, we have very little idea what might be
the cause of the unexpected behaviour.
...
Tables~\ref{tab:octane} shows the full Octane results. These are notably less
