...
that we the data we obtain high-quality data \jasper{that sentence doesn't make sense}, we have carefully designed our
...
benchmarked; and the \krun system we developed to run benchmarks.\jasper{use , instead of ; ?}
...
\jasper{What is the purpose of this sentence? To show what awesome tools we can write?}

In the rest \jasper{remaining?}
of this subsection, we describe: the variables which \krun controls, both
platform neutrally \jasper{just neutral?} and platform specific; and how it collects iterations data.
...
benchmarking. \jasper{Too casual?}
...
\jasper{Raises the question whether we haven't done proper cooling.}
...
machines.\cfbolz{I know that it would be slightly annoying to do, but can we
have the same axes for the same benchmarks here?}\laurie{agreed: we really need
to do that}\edd{Sarah is working on this.}}
