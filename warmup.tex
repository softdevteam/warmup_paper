...
an APERF count per second proportional to its base clock rating \edd{this
happens to be the case for our two linux machines, but in general the rate of
increment of APERF is undefined. We need to tell readers to check the increment
on a per-machine basis} \laurie{why don't we define the idle count in terms of
the cycle counter then?}\edd{I think we could, but that's not what we do
currently. If we make that change we would have to make some Krun changes and
restart the benchers. Let me know}. (i.e.~a 3.6GHz processor
...
switching between speedstep values in-iteration)}. Based on this, we define the
...
iteration timings are clearly not. \laurie{RK: I'm not happy with this sentence as PELT 
...
penalty value~\cite{antoch97effect} \laurie{it would be nice if we could give the reader some 
assurance that our penalty value is ``larger'' (than what?) RK: a common penalty used is 4logn 
which is the Schwarz information criterion so we can use that as the ``larger than'' basis if you 
want.}. We manually inspected a large number of
...
produce are generally a \laurie{``small''? RK: I don't we need to quantify this.  The confidence 
intervals are small enough that I think we are leaving ourself open if we quantify it.} over 
approximation. We ran ran a simulation study with similar dependencies to those observed in our 
...
this.  Unless you guys have a specific example or examples?}. Across 
1,000 simulations only 2 had smaller confidence intervals than when the data were independent
\laurie{were the other 998 smaller or the same or a mix?, 998 had larger confidence intervals}. 
Overall, despite
