...
precisely the same way on each of our benchmarking machine. A possible solution
to this problem would be to use a
\kalibera\cite{kalibera12quantifying,kalibera13rigorous} style experiment in
which one of the repetition levels varies the benchmarking machine. It is not
clear however, whether the \kalibera method caters well for benchmarks which do
not seem to warmup: in such cases the method suggests that a single in-process
iteration sample should be taken consistently across all process executions.
This strategy assumes that the chosen sample is drawn from similarly behaving
process executions, which, as our results show, is not necessarily true.
\laurie{if we're lucky, our cross-machine results
...
frequency is fiction for Intel Core processors'' \laurie{turn this into a bib
entry https://www.kernel.org/doc/Documentation/cpu-freq/intel-pstate.txt}. In
...
specified in the soft \laurie{and hard?} ulimit).
...
feature). While this removed noise from some some VMs, it slowed others down by
