...
cover the `true' mean. However, since we can't tell when
...
no steady state process executions must be handled specially.
Consider a \vmbpair pair $P$ from the original results and a \vmbpair pair $P_s$
sampled from $P$. If $P$ and $P_s$ are consistently no steady state then the
classifications comparison succeeds, but we do not count the steady iter (\# or s) and
steady perf (s) as either succeeding or failing (since no meaningful comparison can be made).
If $P$ and $P_s$ both contain at least one no steady state process execution,
but one of $P$ and $P_s$ is not consistently no steady state, then
the classifications comparison succeeds, and we again do not count the steady iter (\# or s) and
...
iter (\# or s), and steady perf (s) comparisons all fail.
...
To determine whether the sample sets of size $n$ would be statistically equivalent
to the original, we checked whether the confidence intervals for their classification,
steady iter (\# or s), and steady perf (s) overlap with the confidence intervals
for the original data ($n=30$).
One unintuitive side effect of this sampling when
combined with our rules for no steady state process executions is that
the $n=30$ sample does not lead to 100\% similarity. Consider a \vmbpair pair
which has 29 warmup process executions and 1 no steady state process execution.
Whenever a sample of \numpexecs warmup process executions is created, then all three
comparisons (classification, steady iter (\# or s), and steady perf (s)) will fail,
...
Our analysis shows, on our results at least, the importance of running multiple
process executions: at $n=3$, for example, the quality of results is relatively
poor; but at $n=6$ one can clearly observe the beginning of the point of
diminishing returns. Based on these results, we suggest that 16 process
executions is a reasonable, and realistic, figure for most
cases, \sarah{for most cases or just the VMs we have tested?} yielding similar quality results to the \numpexecs that we ran. As with in-process
