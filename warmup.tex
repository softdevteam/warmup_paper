...
\edd{comments on the algorithm: All presentational nitpicks.}
\edd{having seen the variable name ABSOLUTE\_DELTA, I wonder if BOUND\_PROPORTION should be RELATIVE\_DELTA?}
\sarah{A proportion and a delta are different things...}
\edd{``i.e. 0.1\%'' of what? Final seg, right?}
\edd{``Number of in-process iterations to be considered a steady state'' but must be at the end of the pexec}
\edd{``Given an *ordered* list of Segments *"rsp"*'' -- what does ``rsp'' stand for by the way? Why not something really obvious like ``segs''}
\edd{``continue'' Add comment: ``segment is equivalent to the final segment''}

\sarah{The argument is a run sequence, but not really run-sequence plot, I would call the variable 'seq' rather than 'rsp'.}
\sarah{If this is pseudocode you might as well use 'class' for the string variable.}
...
one another.
\sarah{Confusing, maybe "separated by another, distinct, segment." or some such.}
...
\sarah{GB spelling is "in practice"}
...
problems by allowing a tolerance when comparing two segments' means
\edd{that solves one of the two problems. We should brielfy mention how we
allow segments to be ``separated''}: we consider
...
\edd{something like: Note that $j - i$ may be $>1$ meaning that equivalent
segments may be separated by other segments}.
...
in-process iterations are considered equal \edd{you used the word ``equivalent'' earlier, which I think captures our intention better than ``equal''} to the final segment. If
...
\sarah{equivalent, not equal, again.}
...
segment, leading to a classification of \emph{warmup} (\warmup)
\edd{I don't think that is strong enough. The final set of equivalent segments
must be the fastest. Just becuase one earlier segment is slower doesn't mean
the final segments are the fastest, right?}. An executable \sarah{pseudocode?}
...
bracketted prose. E.g. $\langle$VM, benchmark$\rangle$} pair as follows:
...
means that, at least sometimes, users will experience bad performance \edd{s/performance/warmup behaviour/?}.
\sarah{In the PDF it is difficult to see the tick in the good inconsistent symbol, maybe just use the old inconsistent symbol for good, and the cross for bad?}
