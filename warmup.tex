...
\section{Investigating Warmup}

In order to test understand VM warmup, we present a series of experiments. The
main experiment tests Hypothesis H1. After carefully running a number of
benchmarks on a number of VMs, we present a new statistical method for
automatically determining if/when warmup has occurred, classifying benchmarks
based upon that, and presenting steady-state in-process iteration times (when
possible) based upon those. As a useful bonus, we use this experiment to present
measurements for \emph{startup}: how long it takes until a VM can start
executing any user code. We then take a subset of VMs, and record how much
Garbage Collection (GC) and JIT compilation contribute to each in-process
iteration's execution time, hoping to gain insight into why VMs lead to
unexpected process execution data. Finally, we show that our statistical method
can be applied to well known benchmark suites run in a conventional manner.
