...
\laurie{right?}\edd{Yes, but I don't think these implementation details
important. It's probably enough to say that the results are not in-memory
during benchmarking, then we have more space for more important stuff.}, so
that, to the extent that we can control, \krun runs
in
constant memory before benchmark running\edd{The file is never entirely read
in. The reason the fields are fixed-size is so that we can update the file
without skewing the offsets and having to rewrite the rest of the file out.
Let's kill this para? It's not important for the reader to know these
details.}. Note that after a benchmark has run,
...
\edd{No, disabling the driver stops the Linux kernel requesting P-state transitions.}
...
complete by in-process iteration \edd{XXX}}
...
\caption{An example of cyclic behaviour. The cycle is also evident in the core-cycle count.}
