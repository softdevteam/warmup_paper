...
dataset is the number of \vmbpair pairs for which \emph{all} characteristics are statistically
...
250). Thus, roughly speaking, for each $n$ we produced a new summary table which was
compared to Table~\ref{tab:mainresults} for similarity. We also computed an
`overall' similarity score which is the number of \vmbpair pairs for which all three
characteristics (classifications, time to reach steady state and steady state performance)
are statistically equivalent to the full experiment.
...
\caption{\sarah{Check 1000 trials before submission}How the \bencherseven
classifications would have looked if we had run fewer than \numpexecs process
executions. From the original \bencherseven dataset, we created $1000$ samples
...
each truncated dataset is the number of \vmbpair pairs for which \emph{all}
...
To understand whether we could have achieved similar quality results with fewer process
executions, we took the \bencherseven data set and created 1000 samples of $n$
process executions, sampling with replacement, where $n \in \{2, 3, \ldots, 29, 30\}$.
We then repeated the changepoint analysis \laurie{why? i thought we'd just need to redo the IQRs and CIs of the steady state stuff}, and compared the resulting
classifications, times to reach a steady state, and steady state performances to
the original ($n=30$). We compared each characteristic (classification, steady
state performance, etc.) in the sampled set to the original by checking for
overlapping confidence intervals (or IQRs in the case of times to reach a steady
state). For classification data, we constructed multinomial proportion confidence
intervals, using the method of \cite{sison95simultaneous}, to compare the proportion
of warmup, slowdown, flat and no steady state classifications in each sample set
with the original.
As with the truncated in-processs iterations analysis,
we effectively produced a new summary table which was compared to Table~\ref{tab:mainresults}
for similarity. We also computed an `overall' similarity score which is the
number of \vmbpair pairs for which all three characteristics (classifications,
time to reach steady state and steady state performance) are statistically
equivalent to the full experiment. Figure \ref{fig:pexecstruncated} shows the
