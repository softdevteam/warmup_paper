...
\sarah{Potential reviewer question: why did our temp check not catch the overheating?}
...
\sarah{Last minute: need to sweep for wallclock vs. wall-clock.}
...
Hypothesis H1\edd{does this macro include bad but consistent?}.
...
\edd{Revise once we have the correlation analysis rounded off}
...
overall search space\edd{Can we sneak in that many languages the addresses of
alllocations are exposed to the VM (and in some cases to the user-program), so
it's entirely possible that turning ALSR off restricts control flow through
both?}
