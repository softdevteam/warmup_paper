...
These results are significantly worse than reported in previous works, and
suggest that much real-world VM benchmarking is likely to be partly or wholly
misleading. This has real-world consequences as microbenchmarks similar to those
in this paper are often used to determine the efficacy of VM optimisations:
...
The \krun tool we wrote controls a large number of confounding benchmarking
variables, reducing the chances that external factors explain our results. For
example, \krun reboots machines before each process execution, turns off network
cards and stops userland daemons before benchmarking, and ensures that the
...
has completed. \kalibera convincingly show the limitations of such approaches
\cite{kalibera13rigorous}, suggesting instead a manual approach to determining
if and when warmup has completed. While this is a significant improvement on
previous heuristics, it is time-consuming and prone to error. We first show how
changepoint analysis can be used to automate the classification of process
executions, allowing us to reliably classify large numbers of process
executions. We then present summary statistics which allow us to gain insight
into the large amount of data our experiment produces.

In order to try and understand our data, we take two VMs and separate out the
effects of garbage collection and JIT compilation. This only explains part of
the odd effects we see. Neither we, nor the several VM teams we have spoken to,
have any easy explanations for the remaining data.
...
often fail to warmup.
    \item We present a methodology for automatically classifying VM benchmark
data on a par with the most sophisticated manual approach.
...
BOUND_PROPORTION = 0.001 # Proportion (i.e. 0.1%)
ABSOLUTE_DELTA   = 0.001 # Time in seconds
STEADY_STATE_LEN = 500   # Number of in-process iterations to be considered a steady state

# Given a list of Segments -- each of which has "mean" and "end" attributes, the latter its absolute ending
# position in the sequence of in-process iterations -- return a string classifying the run-sequence plot.
def classify(rsp):
  lower_bound = min(rsp[len(rsp) - 1].mean * (1 - BOUND_PROPORTION), rsp[len(rsp) - 1].mean - ABSOLUTE_DELTA)
  upper_bound = max(rsp[len(rsp) - 1].mean * (1 + BOUND_PROPORTION), rsp[len(rsp) - 1].mean + ABSOLUTE_DELTA)
  cls = "flat"
  i = len(rsp) - 2
  while i > -1:
    cur_seg = rsp[i]
    if cur_seg.mean >= lower_bound and cur_seg.mean <= upper_bound:
      continue
    elif cur_seg.end > len(rsp) - STEADY_STATE_LEN:
      cls = "no steady state"
      break
...
one another. To understand the `noise floor' issue, it
...
two sequences, resulting in two changepoint segments. However, we know that in practise such a tiny absolute timing
...
in action), the third 1.0002 seconds. From our perspective, the first and second
segments are best considered as being equivalent, even though they are separated by a
segment which is clearly different. Our classification algorithm solves these
problems by allowing a tolerance when comparing two segments' means: we consider
two segments $s_i$ and $s_j$ (where $j > i$) to be equivalent if $s_i$'s mean is
within 0.1\% or 0.001s (whichever is greater) of $s_j$'s mean.
...
in-process iterations are considered equal to the final segment. If
...
segments are considered equal, leading to a classification of \emph{flat} (\flatc);
...
slowdown, then by definition at least one segment must be slower than the final
segment, leading to a classification of \emph{warmup} (\warmup). An executable
version of the underlying algorithm is shown in Figure~\ref{lst:classification}.
...
Based on this, we can then classify a (VM, benchmark) pair as follows:
...
means that, at least sometimes, users will experience bad performance.
