...
samples (using sampling with replacement) of size $n$\edd{so you get $n$ new process execs I think? It's not totally clear}.  One unintuitive side effect of sampling is that
...
comparisons (classification, steady iter (\#), and steady perf (s)\edd{we are omitting steady iter(s) as it's the same as \#?}) will fail,
