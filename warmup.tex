...
3*(90\%\textrm{ile} - 10\%\textrm{ile})$. We ignore the first window of 200
in-process iterations, as we want to avoid classifying a small number of slow
iterations at the start of the series as outliers, since these may be an
indication of good warm-up behaviour. Of the \laurie{XXX} in-process
...
determine when the time taken by in-process executions to complete have become
faster. Put another way, we expect to see a number of in-process
executions taking time $t$ to be followed by a number at time $t'$, where
$t>t'$. In order to
...
We pass a \emph{penalty} to the function of $15\log{N}$, where $N$ is
the length of the time series once outliers have been removed.
\texttt{cpt.meanvar} returns changepoint locations and the mean and variance
...
left-to-right) whose confidence intervals overlap, and/or whose means are within
0.001s of each other. We use inter-quartile range as a confidence interval, as
it is a conservative statistic which does not assume an underlying distribution
of the data.
...
the shortest execution time of all segments is classified as a \emph{warmup}
(\warmup). A steady-state process execution whose final segment is not the
shortest execution time of all segments is classified as a \emph{slowdown}
(\slowdown).
