...
a single physical processor chip counts as a CPU.} 250 times (`ticks') a second
to decide whether to perform a context switch. Tickless kernels with the
\texttt{CONFIG\_NO\_HZ\_FULL\_ALL} option set run in adaptive-tick mode:
only CPUs with more than one process scheduled on them are interrupted by
ticks. When we compared a subset of benchmarks on a tickless vs.~a standard
kernel, we noticed a small reduction in jitter, although not enough for us to
conclusively conclude that the tickless kernel was the cause of this. However,
since the tickless kernel, at worst, appears to have no negative effects, we ran
