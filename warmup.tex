...
`potentially multiple changes' mean? Just that you can get 0, 1, or more changes.  Most cp 
algorithms only do 0,1 changes so we usually mention that we can find multiple} potentially 
multiple changes. The function
outputs \laurie{can we say `returns'? Yes} changepoint locations and estimates
\laurie{why are they estimates and not precise? Estimates because we don't know the truth.  You 
can just remove estimates and just put ``mean and variance between changes''} of the mean and 
variance
