...
However, we were unable to run some benchmarks. The \texttt{batik} benchmark
failed with a \texttt{InvocationTargetException}, \texttt{eclipse} and
\texttt{tomcat} failed their own internal validation checks.
We also disabled tradesoap, which
failed its internal validation check during our first `full' run on HotSpot:
...
were not able to find a lighter-weight timing mechanisms, despite trying
