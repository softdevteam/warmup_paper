...
\caption{How our experiment would have looked if we had run fewer than
2000 in-process iterations. We artificially truncated process
executions (with in-process iterations from $0 \ldots n$ where $150 \leq n \leq
1950$) and compared their classifications to the original (with $n=2000$) on
\bencherseven. In general, lower values of $n$ lead to more process executions
being classified differently. However, there is a very significant kink in the
data between $1000 \leq n \leq 1050$, almost entirely caused by JRuby (see
...
\end{minipage}
\caption{\laurie{ah, these are unfortunate examples as the original was warmup.
we really want a pexec where $n=1000$ has the same classification as $n=2000$
but different at $n=1050$}Comparing the changepoints and classifications of a
single JRuby process execution originally classified as \laurie{XXX} when
truncated at $n = 1000$ (LHS) and $n = 1050$ (RHS). Many JRuby process
executions have a performance spike in between in-process iterations 1000 and
1050. \laurie{``Since the performance spike is entirely truncated at $n=1000$,
the resulting classification remains the same as at $n=2000$. At $n=1050$,
enough of the performance spike is present to cause a new changepoint segment
that leads to a no steady state classification.''}}
\label{fig:truncated}
\end{figure}
...
around this point. With that in mind, it then made sense to run benchmarks
substantially past that point so that we would be able to see the long-term
effects of any such counters.
...
slowdown). Figure \ref{fig:truncated} shows an example of this phenomenon.

What this in mind, it seems fairly clear that the quality of results from a smaller
...
but at $n=1050$ 21\% would have. However, a reasonable compromise would seem to involve
using smaller numbers of in-process iterations (e.g.~500 in-process iterations) most
