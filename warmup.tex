...
uses simple analyses to guess when warmup has completed,
...
and, if it is, at which in-process iteration the
...
second). Second, we need to decide an allowable tolerance around the desired APERF/MPERF ratio of 1.
For the 4790 processor, the speedstep values either side of its base frequency of 3.6GHz
...
safe APERF / MPERF ratio to be $1 \pm 0.025$\edd{I actually used the exact
lower and upper bounds determined specially for each machine (and thus,
a-symetric bounds for bencher7). Bencher5: [0.944, 1.056], bencher7: [0.943, 1.114]}.
The checking tool then examined
every process execution on \bencherfive and \bencherseven, checking that every
active core had a safe APERF/MPERF ratio. Of the 5,520,000 in-process iterations
on these two machines, 209 (0.008\%) \edd{sorry, it's actually 210. I missed
one decimal place on the bencher7 ratio, and when fixed adds one more (also an
outlier).} (spread over 6 process executions) failed
this check, all on \bencherseven. Manual inspection confirms that these
iterations also have higher wall-clock times, which is further evidence that the
processor briefly throttled itself. Fortunately, outlier detection is designed
to deal with such aberrations, and 205 \edd{206, sorry}(98\%) of these in-process iterations are
detected as outliers, meaning that they do not affect changepoint analysis and
the statistics we produced in any way. The 4 in-process iterations which are not
detected as outliers are contained in 1 process execution of Graal running
\fasta: manual inspection shows that the 4 in-process iterations are fairly
evenly spread over the (fairly noisy) process execution and that one of them led
to a small changepoint segment that causes the process execution to be
classified as no steady state rather than warmup. As this \vmbpair was
\badinconsistent(29\warmup, 1\nosteadystate), it is likely that this
\vmbpair would otherwise have been classified as warmup (as indeed it was
...
\citet{georges07statistically} also present an automated method for detecting
peak performance, which looks for in-process iterations where the Coefficient of
Variation (CoV)\footnote{the CoV is the standard deviation divided by the mean of a sequence of measurements.}
falls below a threshold\footnote{\citet{georges07statistically} suggest 0.01 or 0.02.}.
They then extract a fixed number of in-process iterations as the steady state
of the process execution, whereas our method takes as many iterations as possible,
over a fixed minimum (500). Applying the CoV method to our data (threshold 0.01),
we found that it detected a steady state in almost all process executions that we
had classified as having a steady state, but also marked many of our `no steady
state' executions as reaching peak performance (60.0\%  on \bencherseven, 76.7\%
on \benchersix and 83.3\% on \bencherfive). This included some surprising examples,
such as the execution in Figure~\ref{fig:examples:nosteadystate}.
...
words, at worst our confidence values are an over-approximation. Overall,
...
to have helped give the study of warmup a firmer base. The tools we developed
are open-source and can be used by VM developers and users to investigate the
