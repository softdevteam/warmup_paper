...
force a collection after each iteration).\cfbolz{I don't get the last sentence. for the avoidance of which doubt?}
...
second). Second, we need to determine the how far from an APERF/MPERF ratio of 1.
...
safe APERF / MPERF ratio to be $1 \pm 0.025$. The checking tool then examined
every process execution on \bencherfive and \bencherseven, checking that every
active core had a safe APERF/MPERF ratio. Of the \laurie{how many iterations on
b5 and b7 combined?}, 15 failed this check, all on \bencherseven. In each case,
the APERF/MPERF ratio was below 1, suggesting processor throttling. Manually
checking the core-cycle plots showed that in each case, the benchmark had
migrated cores between, or during, the in-process iterations in question. Thus
it is highly likely that our measurements are simply showing the effects of
moving to an idle core that is in low-power mode, with the consequent small
delay required to run at full speed. \laurie{Would
it be correct to say something like ``This effect may have been lessened
if we had turned off C-state in the bios''. can we even do that on b7? i
think we can but i might be wrong} Fortunately, such cases are rare and
are unlikely to have made an impact on our analysis: 14 of the 15 were marked as
outliers by our approach, and thus played no part in changepoint analysis. 
