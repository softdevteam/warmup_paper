...
\edd{this is bencher5's flat and warmup percentages summed. I was expecting
`good inconsistent' to be included too, as (perhaps confusingly) `good
inconsistent' pexecs all consistently reach a steady state of peak performance.
If you do decide to adjust this, look for other percentages that would need
updating.}\laurie{you're right: sarah can you update the macro definitions?}
\end{abstract}
...
\end{enumerate}
\edd{Did you want to mention the startup experiment as a contribution here?}\laurie{i'm in two minds. it doesn't feel like a major contribution, so i'm a bit nervous about muddying the story. thoughts?}\edd{I think you might be right. Let's not distract the readers}
...
kernel~\citep*{tickless}\edd{weird cite, should we use `citepalias' bib command
to make it say (Linux 2016) instead?}, which requires recompiling the kernel
with the
...
iterations. Of the \totaliterations\xspace \edd{ideally the auto-generated macro would use commas to make this number easier to parse}in-process
...
DELTA = 0.001  # Absolute time delta (in seconds) below which segments are always considered equivalent
...
\result\% of \vmbpair pairs have consistently `good' warmup (i.e.~are flat, warmup,
or good inconsistent\edd{or warmup too presumably!}\laurie{yes, the macros might also need updating}) and
...
\edd{something seems wrong with the `good warmup' percentages in this section.
They appear to be the sum of consistent warmup, and consistent flat, but
earlier on we defined `good warmup' including `good inconsistent` too (although
there is an outstanding comment on the exact definition, since it seemed to
exclude consistent warmup).}
...
suite for JavaScript~\cite{octane}\edd{another weird citation} (JavaScript). Octane was
