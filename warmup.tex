...
\emph{flat} ($\rightarrow$). Since all our benchmarks run for 2000 in-process
...
($\rightsquigarrow$). A steady-state process execution whose final segment has
the lowest execution time of all segments is classified \emph{warmup}
($\uparrow$). A steady-state process execution whose final segment is not the
...
\emph{inconsistent} ($\rightleftarrows$).
