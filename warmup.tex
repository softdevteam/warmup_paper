...
$n$ is the time series length minus the number of outliters) to the
...
benchmark which is \inconsistent(5\slowdown, 5\nosteadystate) (i.e.~inconsistent,
with 5 process executions classified as slowdown, and 5 classified as no steady state) is clearly `bad';
but a benchmark which is \inconsistent(8\warmup, 2\flatc) is likely to be
...
\caption{Benchmark results for \bencherthree. Classifiers key: \flatc: flat,
\warmup: warmup, \slowdown: slowdown, \nosteadystate: no steady state,
\inconsistent: inconsistent. For processes which reach a steady
...
iterations. Table~\ref{tab:mainresults} shows the results from \laurie{bencherX}.
\laurie{we should summarise: X\% are flat, Y\% are warmup etc.} Of the
\laurie{XXX} (VM, benchmark) pairs, \laurie{XXX}\% are flat, \laurie{XXX}\%
warmup, \laurie{XXX}\% slowdown, \laurie{XXX}\% are no steady state, and
\laurie{XXX}\% are inconsistent. Of the \laurie{XXX} process executions,
\laurie{XXX}\% are flat, \laurie{XXX}\% warmup, \laurie{XXX}\% slowdown,
\laurie{XXX}\% are no steady state, and \laurie{XXX}\% are inconsistent. These
results clearly invalidate Hypothesis H1. The results for \bencherfive and
\benchersix can be seen in Appendix~\ref{app:mainresults}; \bencherfive's results
are very similar to \bencherthree's; \benchersix's results are broadly similar.
We consider this strong validation of Hypothesis H2.

A few aspects of Table~\ref{tab:mainresults} are worth particular comment.
First, of the 6 benchmarks, only \nbody has `good' warmup behaviour on all
machines: the other 5 benchmarks all have at least one VM which consistently
slows down. Second, there appears to be a strong bimodal pattern for the number
of in-process iterations required to reach a steady state (when it exists): VMs
tend either to reach it very quickly (around 10 or fewer in-process iterations)
or take hundreds of in-process iterations. Finally, there is very little
...
which take a long time to warmup are using it to create highly efficient
steady-state code.

To understand why we see so many unexpected cases, we created a series of plots
for all process executions on all machines. All
...
\label{aperf/mperf check} The APERF/MPERF ratio plots (see the Appendix for
...
Section~\ref{aperf/mperf error}) and we can't expect to always see a
value of precisely 1. We wrote a simple automated analysis that checks the
...
APERF/MPERF ratio between $1 \pm 0.015$. All active cores in our experiment
...
do not always respect operating system requests. However, the APERF/MPERF
check of Section~\ref{aperf/mperf check} strongly suggests that our main
experiment was not subject to any significant CPU frequency changes.
...
as well as the filesystem overhead, reading from a MSR device
...
\noindent In this appendix, we first present complete data from our experiment
(Appendices~\ref{app:mainresults}, \ref{app:dacapo}, and \ref{app:octane}). We
