...
by better approaches in the future. However, even this current work is
sufficient to show that there are clear problems with previous approaches to VM
benchmarking, including those we personally have been involved with in the past. In this
section we make some tentative suggestions to the VM developer and user
communities based on our experience.

First, our results show that many benchmarks take considerable time to reach a
...
reach a steady state. We can thus no longer assume that a steady state is
reached after a fixed number (no matter how high) of in-process iterations, or
that one process execution tells us about the rest. Each process execution's
data must be analysed individually to determine if, and when, a steady state is
reached. The fully automated changepoint analysis-based approach presented in
this paper is one possibility, and is is available to everyone in our
open-source tools.

Second, we believe that the traditional practise of presenting only steady state
numbers is hard to defend, as it tells us too little about the real-world
performance that users will experience. We hope that future VM experiments will
present: a benchmark's classification (warmup, flat, slowdown, no steady state);
and, when appropriate, the point at which the steady state was reached (using
one or both of steady state iter (\#) or (s)) and peak performance.

Third, simply running benchmarks for a larger number of in-process iterations
helps one understand a VM's long-term performance stability. Traditional VM
...
many process executions are needed to uncover inconsistencies. We suspect that
10 process executions is a reasonable, and not too onerous, figure.
