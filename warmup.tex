...
After running the benchmarks as described, we analysed the results in two
phases. First, for each process execution, we visually inspected a run-sequence
plot (iteration  number vs. time). For each execution we noted whether the
pattern of iteration times matched the expected warmup behaviour (as described
in Section~\ref{sec:warmup}). This was the case of \cfbolz{add numbers} X out
of Y benchmarks. Next we identified groups of ``anomalies'', i.e. common
deviations from the expected warmup behaviour. Since we don't yet have formal
definitions for these classifications, we define them
qualitatively.\footnote{In the future, we hope to develop formal definitions so
that this process can be automated.} In the
following subsections, we first show some example where our benchmarks did
perform as expected. We then discuss our anomaly classifications and give
examples.

\subsection{Traditional Warmup}

Many benchmarks do indeed warmup as expected, although the length of the warmup
phase is quite variable. Figure~\ref{fig:examples:trad} shows two process
executions which conform to the traditional warmup model: one warms up after
one iteration, the other in roughly 120.\edd{I hope to find a better example
than the second: one with more obvious compilation steps}
...
We loosely define outliers as iterations which take much longer (or shorter) than the
other iterations, even taking the randomness of the benchmark iterations into
account.
Figure~\ref{fig:examples:outliers1} shows one such example, which includes a
slow outlier around iteration $1650$.\edd{speak about consistent outliers}
...
A few benchmarks exhibit `slowdowns', where later iterations perform worse than
earlier iterations. Figure~\ref{fig:examples:slowdown1} shows an example. The
large `step' at around iteration 200 is a slowdown. Slowdown need not manifest
as a `step': it could also manifest as a gradual drift towards slower
iterations times.
...
Some benchmarks exhibit cyclic behaviour, where there is a repeating
pattern in iteration times. We observed varying lengths of cycles in our
results. For example, Figure~\ref{fig:examples:cycles} shows two different
process executions with cycles of length roughly 7 and 400
respectively.\cfbolz{the second figure is missing}
...
\caption{Example of a benchmark with a late phase change (in this case entering slowdown).}
...
clear that some benchmarks change phase after a much larger number of iterations
than previous experiments have considered (see Figure~\ref{fig:examples:late1}).
Late phase changes should be considered with our classifiers: after a late phase
change, a benchmark may run faster, slower, or become cyclic etc.


\subsection{Never-ending Phase Changes}
\label{sub:long}
...
\makebox[\textwidth][c]{
\begin{minipage}{.65\textwidth}
\includegraphics[width=\textwidth]{examples/late1}
\caption{Example of a benchmark with never-ending phase changes.}
...
Figure~\ref{fig:examples:inconsistent} shows some such cases.

\begin{figure*}[h!]
\makebox[\textwidth][c]{
\begin{minipage}{.65\textwidth}
\includegraphics[width=\textwidth]{examples/inconsistent1a.pdf}
\end{minipage}
\begin{minipage}{.65\textwidth}
\includegraphics[width=\textwidth]{examples/inconsistent1b.pdf}
\end{minipage}
}
\makebox[\textwidth][c]{
\begin{minipage}{.65\textwidth}
...
\caption{Examples of benchmarks with inconsistencies between process executions
and machines. The top row shows inconsistencies between different process
executions on the same machine. The bottom row shows inconsistencies between
machines.\cfbolz{I know that it would be slightly annoying to do, but can we have the same axes for the same benchmarks here?}}
