...
timing data collected before warmup is complete is discarded, therefore
...
combination) of \vmbpair pairs \edd{I wonder if we should be saying ``\mvmbtrip triples`` (as we have done in a couple of other places) to make it clear that it's ``this vm, this benchmark, on this machine''} consistently reach a steady state of peak
...
than expected by VM teams we have spoken to\edd{Although true, too anecdotal?}. Our results suggest
...
\edd{not sure it's worth using subsections in the intro}
...
\edd{the flow between this para and the previous is quite abrupt.}
...
indication as to whether the steady state is indicative warmup or not
(regardless of whether the in-process iterations of the steady state are the
fastest). We
...
the benchmark). This allows us to process our large data-set and
...
\begin{enumerate}
    \item We show that, even in the most idealised setting\edd{too strong? Ideally, we would have used a system without p-states, and we would have been able to pin properly, ...}, simple benchmarks
...
reached.
    \item We present the first analysis to identify different classes of
