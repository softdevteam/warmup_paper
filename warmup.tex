...
guarantee that they all reach a steady state by that point. Although
in-process iterations which have a predictable performance pattern are
implicitly classified as `cyclic`, \kalibera do not otherwise classify
...
User-configurable commands can be run before and after benchmark execution.
We disabled as many Unix daemons as possible (e.g.~smtpd,
crond) to lessen the effects of context switching. We also
turned off network interfaces entirely, to prevent outside sources causing unbounded
...
effect of many external events can add up: we use a changepoint segment's variance
as a proxy for this proportional notion of tolerance (ignoring that
the variance's units are $\textrm{s}^2$). We therefore combine these two notions,
...
are independent. Across all three of our benchmarking machines, 11.8\% of process executions
...
2000 in-process iterations. We truncated process executions (taking in-process
iterations $0 \ldots n$ where $n \in \{1990, 1980, \ldots, 10\}$) and compared
...
in-process iterations from $0 \ldots n$ where $10 \leq n \leq 1990$ in
increments of 10), repeated the changepoint analysis, and compared the resulting
...
250). In essence, for each $n$ we produced a new summary table which was
...
notable kink is between $1000 \leq n \leq 1400$. This is almost entirely caused by
