...
We use the recently introduced concept\edd{Is it a concept? Feels more like a
method or a process} of changepoint analysis~\cite{eckley11analysis} to
...
We therefore use the concept\edd{Is it a concept?} of changepoint analysis
...
There are various ways that one can define a changepoint, but the best fit
...
published benchmarks that run for 0.001s or smaller: it is very easy for
external events (e.g.~an unexpected context switch) to substantially impact such
measurements, giving one a false impression of performance. As a rule of thumb,
we suggest that the fastest VM should take around 1s to run a single in-process
iteration. Sometimes other VMs run the same benchmark so slowly that this is
impractical, at which point it may be necessary to relax this to 0.1s, but we
strongly discourage running times lower than that.
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.485\textwidth]{examples/truncated_same_plot.pdf}
\caption{The percentage of classifications that match our original 2000-iteration
experiment on \bencherseven, for a number of iteration sizes (from 150--2000).}
\label{fig:sametruncated}
\end{figure}

What allowed us to spot the issues noted in this paper was simply running
benchmarks for a longer period than suggested by previous methodologies.
...
runs are insufficient. However, no-one wants to run benchmarks for any longer
than necessary to collect reliable statistics. Assuming that we ran
...
values. Although many are typically set to small values, we suspected that some
counters would be set to 1000 (or, to make things look more `computer friendly',
to 1024) meaning that we might observe VM effects at in-process iterations
around this point. It thus made sense to run benchmarks substantially past that
point so that we would be able to see the long-term effects of any such
counters.

To understand if we could have achieved similar quality results with a shorter
running time, we performed changepoint analysis on each process execution's first
...
monotonically. Ideally one would pick a value of $n$ that has a good trade-off between accuracy and running
time.
...
JRuby, many of whose process executions have a distinct performance spike
...
leading to a classification of no steady state (or, less often,
slowdown). Figures \ref{fig:truncated1} and \ref{fig:truncated2} show
...
\caption{Process execution 2 of the warmup experiment on \bencherseven,
truncated at 1000 iterations.}
\label{fig:truncated1}
\end{minipage}
\hfill
\begin{minipage}[t]{0.485\textwidth}
\includegraphics[width=\textwidth]{examples/truncated2.pdf}
\caption{Process execution 2 of the warmup experiment on \bencherseven,
truncated at 1050 iterations.}
\label{fig:truncated2}
\end{minipage}
\end{figure}

What this in mind, it seems fairly clear that choosing a smaller
...
course of action. It is thus difficult to draw definitive conclusions from
our data: at $n=650$ only 15\% of classifications would have changed,
but at $n=1050$ 21\% would have. However, a plausible approach would seem to be to use
