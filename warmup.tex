...
\caption{\sarah{updated!}How the \bencherseven classifications would have looked if we had run fewer than
...
similar to the full experiment.
...
$n$ in-process iterations. For each truncated data set, we generated summary statistics
(of the sort seen in Table~\ref{tab:mainresults}) and compared the classifications, time
to reach a steady state and steady state performance to that of $n=2000$, by checking for
overlapping confidence intervals. We also compute the ``overall'' similarity of each
truncated dataset to $n=2000$, which is the number of \vmbpair pairs for which \emph{all}
characteristics (classifications, time to reach steady state and steady state performance)
are statistically similar to the full experiment (see the black line on Figure~\ref{fig:sametruncated}).
