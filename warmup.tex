...
difficult\edd{I don't understand the previous sentence. Presumably if an exec
warms up OK, then it didn't exhibit the other classifications?}. Of the eight
VMs we looked at,
...
variants\edd{krun already has the notion of a "benchmark variant" used for
composed benchmarks (Python->PHP, PHP->Python etc), but these are not used in
this paper. I wonder if the loose use of "variant" throughout will confuse
readers who read our future papers and see this word used differently. Very
minor perhaps. Could replace with versions/implementations?}
...
wish to measure.\edd{We state that we occasionally saw non-determinism on Java,
but it's not explained where that came from. Te text explains that classes are
lazily loaded inside timed code, and that the cost of loading is observable,
but not where the non-determinism is sourced from.}
...
physical cores in our benchmarking machines\edd{I think we delete this now,
since we don't pin and have VMs with threads?}.
