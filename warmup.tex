...
state, or which perform inconsistently when repeated.
...
some of the effects we see, in many cases it does not.
...
that period. Often $n$ is a fixed number (typically around 5), with little or no indication
as to whether warmup has completed or not. More advanced methods such as
...
\edd{something is missing here. `flat' is a predictable performance
patternwhich is not cyclic}
