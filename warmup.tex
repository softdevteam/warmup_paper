...
on a per-machine basis} (i.e.~a 3.6GHz processor
...
think the speedstep increments are irrelevant}. Based on this, we define the
...
The changepoint analysis algorithm we use assumes that the time-series data is independent, which 
our in-process iteration timings are not. In practise, this is not an
issue as theory in \cite{antoch97effect} dictates that we can inflate out penalty value to 
compensate: we manually inspected a large number of benchmarks and found that the
...
Section~\ref{sec:timings}), we assume that data within segments are independent. In practise, the 
consequences of this are limited for two reasons. First, over 99\% of steady state segments have a 
variance under 0.001s (with the vast majority one or more orders of magnitude smaller than that),
which means that the confidence intervals we produce will be small no matter what we do. Second, we 
ran a simulation study with similar dependencies as observed in our segments we found that across 
1,000 simulations only 2 had smaller confidence intervals than when the data were independent.  
Thus we produce confidence intervals that are larger than under independence.  Overall, despite 
