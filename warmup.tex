...
We first need to exclude idle cores from any checks. Although the rate of
change of APERF \laurie{does it say ``rate of change'' or does it just say something like ``APERF isn't defined at all''? is mperf not defined either? it would seem odd to define one but not the other...} is architecturally undefined~\cite{intel2017pstate}, in
practise a fully busy core increments APERF proportional to the CPU base
frequency (e.g.~ \bencherfive's CPU increments APERF at $3.6*10^9 Hz$).
...
have no real bearing on the ratios we can see}. Based on
this, we define the
