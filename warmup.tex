...
\caption{\laurie{can we make the $x$ axis finish at 2000 not 2100?} How the \bencherseven experiment would have looked if we had run fewer than
...
\end{minipage}
\caption{\laurie{ah, this example has two unfortunate things. First, note the LHS plot doesn't have an outlier at iteration 395(ish). there are good reasons why this might be so (sliding window size), but it won't be obvious to many readers. Second, it doesn't have a performance spike: it actually has a `gets faster' spike after iter 1000. we need to find a pexec without either problem, or we'll confuse readers} Comparing the changepoints and classifications of a
single JRuby process execution originally classified as warmup when
