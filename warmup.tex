...
length of the time series data?} model fits.
...
2000 changepoints, does that mean the complete search space is 2000**26?}.
...
the first changepoint if it's towards the end of the time series data?}. Furthermore PELT includes an extra pruning step: once an
``obvious'' changepoint has occurred it is unlikely \laurie{does `unlikely' mean
``could happen, but it's so rare that it's not worth considering''?} that the
last changepoint locations will be
prior to this ``obvious'' change.  This reduces the computational cost from $\mathcal{O}(n^3)$ to 
$\mathcal{O}(n)$ \laurie{i'm not sure i can quite relate this order of complexity to what came earlier}.
