...
users, it gives no insight into multi-threaded computations: we therefore also record \laurie{on linux only?}\edd{yes, becuase it needs MSR access}
...
than the VM.
inconsistency. Bad inconsistency is always more troubling: it
...
slowdown behaviours. Figure~\ref{fig:examples:inconsistent} shows a case of bad inconsistency.
Figure~\ref{fig:examples:warmup_flat} shows a case of good inconsistency.
examples. Core-cycle plots (Linux only) show the
...
\caption{`Bad inconsistency'. Examples of warmup and slowdown from the same
benchmark, on the same VM, on the same machine.}
...
\caption{`Good inconsistency' occurs because some process executions are on the
edge of our ability to differentiate warmup from flat behaviour. A human
classifier might consider both of these plots to be cases of warmup.}
...
\laurie{edd: we need to reverify this}\edd{OK}In order \edd{We seem to say `in
order' quite a lot. Diversify?}to verify that there were no frequency changes
during benchmarking,
...
experienced no unexpected CPU frequency changes\edd{We don't have the code for
this tool, so I'll probably end up re-writing it. I was hoping to have idle
counts for all the machines at this point, but something approximate will
probably do}.
...
the user to manually pick one part of the cycle for bootstrapping, which leads to a clear potential for
