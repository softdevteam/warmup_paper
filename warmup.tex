...
\krun checks that it is running on a `tickless' kernel. By default,
the Linux kernel interrupts each active CPU \laurie{is it per cpu or per core?
because we later say `boot cpu', which presumably is `boot core', otherwise our
single-cpu (but multi-core) machines will be running in tickful mode!} 250 times
(`ticks') each second to run
scheduling and read-copy-update routines, interfering with running processes.
For all but the boot CPU core, unnecessary \laurie{what does unnecessary mean?}
ticks can be turned off by setting the \texttt{NO\_HZ\_FULL\_ALL}
\laurie{boot-time?} option. We ran a subset of our benchmarks in the default and
tickless modes, and
although we were unable to identify significant jitter, we still opted for
\krun to run on tickless mode \laurie{this sounds really weird. if it doesn't do anyhting, why do we have this option on? i thought that we'd spotted a small amount less jitter?}.
