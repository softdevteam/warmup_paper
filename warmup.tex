...
\jasper{Maybe turn the order around. First describe what kind of benchmarks where chosen and why.
Discuss ``weak points'' and then list the benchmarks.}
...
mode \jasper{isn't it just overclocking?};
this eventually causes the CPU to exceed its safe thermal limit, and
performance is then reduced to a lower \jasper{normal/specified} limit.
...
physical core is in fact more than one core \jasper{I think this is oversimplified, even for our purposes},
allowing more programs to run semi-simultaneously. However, hyper-threading causes programs to interfere
...
overheated; the only indication the system gave was via \texttt{dmesg}.
...
start at (approximately) the same temperature. When initially invoked, after a
configurable `cool down pause' (we used 60 seconds), \krun collects temperature
readings from all available temperature sensors. Prior to running further
process executions, it then waits for each sensor to  cool to within 10\%{} of
its starting temperature. If any sensor fails to cool to within this threshold
...
`adaptive-tick mode' for as many logical CPUs as possible.\footnote{Not all
