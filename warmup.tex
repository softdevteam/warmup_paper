...
\edd{Consider putting the punchline of this para foremost, i.e. ``First, each
pexec must be individually analysed''. Otherwise the para rambles a bit and only at the end does it become apparent what all this is leading to.}
First, our results undermine the previous VM benchmarking orthodoxy of benchmarks
quickly reaching a steady state after a fixed number of iterations. It is clear
...
\edd{optionally say that manual inpsection is out of the window for all but the
...
community's approach \edd{to what?} may apply equally well to VMs: this would involve
...
non-deterministic crashing bugs \edd{I think it's obvious that benchmarks that
crash are bad. I'm not sure we need to mention this} are likely, at best, to
frustrate those benchmarking and,
...
gives the same results as Table~\ref{tab:mainresults}'. We report: individual
...
is true if and only if the three individual comparisons are true.
...
has 30 warmup process executions then the classifications comparison is true,
...
slowdown process executions then the classifications comparison is false, but
...
which has 29 warmup process executions and 1 no steady state process executions.
