...
on a per-machine basis} \laurie{why don't we define the idle count in terms of the cycle counter then?} (i.e.~a 3.6GHz processor
...
think the speedstep increments are irrelevant}\laurie{for the ``perfect'' upcoming version, maybe, but not for this version (which has to be much looser in its tolerances)}. Based on this, we define the
