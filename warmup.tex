...
source of error in any comparison involving confidence intervals. There is no
guarantee that the original mean and confidence interval cover the `correct'
value. Thus sometimes the sampled \vmbpair pair might cover the `correct' value
while the original \vmbpair pair does not. However, since we can't tell when
...
is heavily dependent on the VM(s) in question: if one is confident that the VM(s) in question do not cause
...
of size $n$ from the original \vmbpair pair's process executions.
We then report the proportion of sampled \vmbpair pairs which are statistically
equivalent to the original. One unintuitive side effect of this sampling is that
...
If a sample of 30 warmup process executions is created (which is not unlikely), then all three
