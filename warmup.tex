...
data. We have chosen to define `similar' as `what proportion of the sampled
\vmbpairs have the same results as Table~\ref{tab:mainresults}. We report
individual proportions of \vmbpair pairs classifications, steady iter (\#,
s)\footnote{Since steady iter (\#) and steady iter (s) both report the same
underlying measurement, but in different units, we treat them as one.}, and
steady perf (s). We also report an `overall' measure which is the proportion of
\vmbpairs for which all three individual comparisons are true.

In most cases the comparisons involved are straightforward. Consider
a \vmbpair pair $P$ from the original data and a \vmbpair pair $P_s$
sampled from it. If both $P$ and $P_s$ consistently warm-up,
then: the classifications comparison is true;
and the steady iter (\#, s) and steady perf (s) comparisons depend on comparing the
relevant values calculated from $P$ and $P_s$ (taking into account confidence intervals).
If $P$ is consistently warm-up but $P_s$ is consistently slowdown then:
the classifications comparison is false; but
the steady iter (\#, s) and steady perf (s) comparisons comparisons are made as
...
classifications comparison is true, but we do not count the steady iter (\#, s) and
...
the classifications comparison is false, and we do not count the steady iter (\#, s) and
...
iter (\#, s), and steady perf (s) comparisons are all false.
...
at $n=650$ only 9\% of classifications would have changed,
but 22\% would have changed at $n=1050$.
