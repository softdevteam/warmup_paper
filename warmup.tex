...
\vmbpair pairs consistently reach a steady state of peak performance.\edd{Should this really be machine, bench, vm \emph{triples}? Otherwise we would have one summary figure for all machines, right? Presumably we have a summary range because the summaries are per-machine.}
\end{abstract}
...
\edd{Should another hypothesis be that the steady state is the fastest state?}
...
unpredictably, but which, in large numbers, tend to be lost in the noise\edd{I don't understand the previous sentence}. Put
...
programs)\edd{I think this is not worth mentioning. The field generally accepts that microbenchmarks are already not representative of large programs.}.\edd{A more compelling second point is that once you know that the steady state may not be reached, it becomes much more difficult to even measure anything anymore}.
...
we are able to rule them out as being a significant factor\edd{Let's not mix the findings in with the methodology?}.
