...
\sarah{It would be good to emphasise that changepoints are a relatively new innovation and signal to the reader that this is an important contribution.}
...
We introduce a new approach to analysing VM benchmarking data. We first use
changepoint analysis~\cite{eckley11analysis} to automate the detection
...
performance patterns that previous methodologies have conflated.
...
def classify(seq):
  ...
  lower_bound = min(last_seg.mean * (1 - BOUND_PROPORTION), last_seg.mean - ABSOLUTE_DELTA)
  upper_bound = max(last_seg.mean * (1 + BOUND_PROPORTION), last_seg.mean + ABSOLUTE_DELTA)
  cls = "flat"
  i = len(seq) - 2
  while i > -1:
    cur_seg = seq[i]
