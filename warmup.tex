...
penalty value~\cite{antoch97effect} \laurie{it would be nice if we could give the reader some assurance that our penalty value is ``larger'' (than what?)}. We manually inspected a large number of
...
produce are generally a \laurie{``small''?} overapproximation. We ran
ran a simulation study with similar dependencies to those observed in our segments \laurie{i think we need to give an intuition about what ``similar dependencies'' means. it also seems that two simulations were run, one with dependent data and one with independent data? i'm not sure how the comparison works}. Across 
1,000 simulations only 2 had smaller confidence intervals than when the data were independent
\laurie{were the other 998 smaller or the same or a mix?}. Overall, despite
