...
steady state numbers, due to the assumptions that warmup is both fast and
inconsequential to users, and that the steady state is also peak performance.
...
the checksum is compared to a predetermined expected value; if the comparison fails then
...
roughly the same work, as we share the expected checksum value across all
...
these VMs have not yet been ported to this platform. We skip \fasta on
JRuby+Truffle as it crashes;
...
fragmentation). On its initial invocation (before the first reboot) \krun
...
on any given machine, for each \vmbpair
pairing the additional environment used to configure the VM is of constant
size.
...
classified as non-flat (see Tables~\ref{tab:bencher5results} and
~\ref{tab:bencher6results} in the Appendix)\edd{PyPy and hotspot appear to be a
little more consistent on linux machines. Would you agree?}
...
examples. On Linux we also use \emph{core-cycle plots}, which show the the
...
\laurie{edd: we need to update this section}\edd{I know :)}\edd{Laurie: What do
you think we should we do here? Do you want me to re-do this, or do we now
consider it unnecessary?}\laurie{i think we need to rethink this section a little bit. i think we need to say something, but maybe we can automate it? however, i don't know to automate `partially cyclic'.}
\edd{Yeah, I think the problem is that "cyclic" behaviour comes in many flavors
making it really hard to automate. Maybe it's ok to simply say that cycles with
large enough deviations get classified as no steady state. Other cases could be
`future work' since picking apart the types seems like a large undertaking to
me.}
