...
all other segments are plotted in grey (see
Section~\ref{sec:classifications} for details on segment equivalence).}
...
with a similar mean and variance into distinct segments
...
\end{enumerate}
\edd{Did you want to mention the startup experiment as a contribution here?}
...
DELTA = 0.001  # Absolute time delta between `equivalent' segments
...
independent. Since independence is a prerequisite to bootstrtapping, when
bootstrapping, we sample values within, but never across, segments (e.g.~if a
...
or good inconsistent\edd{or warmup too presumably!}) and
...
process executions\edd{i don't understand the previous sentence. s/good/bad/ ?}. On \bencherseven
