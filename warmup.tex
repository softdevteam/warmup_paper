...
Loading \krun's results file is a relatively memory intensive task which we do
not want to occur before benchmarking (since it is likely to e.g.~fragment
memory). Instead of consulting the results file to decide which process
execution to run next, \krun uses a simple schedule file (created prior to the
first reboot), which is scanned line-by-line for the next process execution.
Only once the process execution is completed, does \krun load the results file
and append the new result. The schedule file is updated before the system
reboots to run the next process execution.
...
each individual processor chip counts as a logical CPU.} 250 times\edd{citation?} (`ticks') per second to
...
process-execution we plot one for each CPU core. A full set of plots (and raw
...
cause of such wall-clock time changes.\edd{can we leave that unsolved? Perhaps
need a graph, either here or in instrumentation section?}
...
an open research challenge in change-point analysis\edd{cite}. Since it is so rare (only 2
...
started executing the user program. The period between invoking a VM and the VM
executing the first line of the user program is the VM's \emph{startup} time,
and is an important component in understanding a VM's performance.

A small modification to \krun enables us to measure startup. For each VM, we
first run a small C program which prints out a starting  wall-clock time
measurement before executing a dummy iterations runner which prints a second
wall-clock time measurement before exiting. We record the delta between these
two clock readings. For each VM we repeat with 200 process executions (for this
measure, there is no notion in-process executions). Figure~\laurie{XXX} shows
the data for each of our
...
timer). Note that both runners run all Octane benchmarks in a
single process-execution\edd{both runners run all Octane benchmarks in a single
process-execution" is there one runner for each vm, or do you mean octane and
dacapo?}\edd{Worth saying why we find this odd? -- ordering of benchmarks will matter}.
