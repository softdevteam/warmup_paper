...
large amounts of memory, though \fasta somewhat less so \laurie{this statement needs checking; my memory of fasta is fuzzy, and it's not totally obvious from the code how much memory it allocs/deallocs})
\edd{max resident set sizes when run under Cpython-2.7: fasta=15832Kb,
binarytrees=8280Kb. Bear in mind that binarytrees is hard-coded to go to a
depth of 12 in our benchmarks. Trees grow exponentially with depth, so there
will likely be a depth where binarytrees would massively overtake fasta}\laurie{hmm, so my memory was wrong ;) can we quickly compare this to a couple of the other benchmarks? maybe the memory allocation of these two is both fairly high, or maybe it's pretty mundane. i'm no longer sure!}.
\edd{spectralnorm=7316kb, richards=7588kb, fannkuch=7028kb}
...
regular GC spikes have only a small effect on the wall-clock times.}
...
as can be seen in Figure~\ref{fig:goodcomp} \laurie{this relationship is now broken: fig:goodcomp used to show an example of where weird behaviour wasn't correlated with GC/JIT, but the current fig:goodcomp shows clear correlation. we need an example that shows non-correlation. that doesn't mean that the current fig:goodcomp isn't interesting, but if we only have room for one figure, we should show the non-correlation one}.\laurie{aha, we could probably place a plot next to table 3, so we could keep all the examples.}
...
\kalibera note the existence of what we have called cyclic behaviour, which
...
the user to manually pick one part of the cycle for
measurement~\cite{kalibera13rigorous}), which leads to a clear potential for
