...
\texttt{tomcat} fail their own internal validation checks\edd{that's only 3, one missing?}. We experienced
...
adequately long running benchmark\edd{fwiw, the main warmup experiment works in the same way}. We recorded \numiterations iterations of the outer benchmark; our runner
...
perform similarly. The luindex benchmark \edd{was not run on graal!} is consistently a slowdown on Graal,
and mostly a slowdown on HotSpot. Jython \edd{and avora and fopp} is notably inconsistent on both VMs. In
...
blips less likely\edd{less noticeable, or less likely?}.
