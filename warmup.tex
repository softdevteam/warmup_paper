...
that period. Often $n$ is a fixed number (typically around 5), with few or no guarantees
that warmup has completed by that point or not. More advanced methods such as
...
inconsistent cases). Second, \binarytrees and \fasta \edd{and richards? that has as many bad inconsistent cases as fasta (4)} seem to be much more often
...
To understand why we see so many unexpected cases we plotted our data. All
plots have in-process iteration number on the
...
meaning that there is no guarantee (that we could see) that it returns within a fixed bound.
...
are independent\edd{repetition, we just said that in the last para}. Across all
...
a benchmark's performance than current approaches.\edd{I don't understand this
final paragraph. If we have space (I think we do), I think we need to explain a
little more clearly what all this means}.
...
constraints) \laurie{is this still true?}, Dacapo was run only on \bencherfive. For both suites we used
