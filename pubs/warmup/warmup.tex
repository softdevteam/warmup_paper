...
\laurie{i propose that we fix kalibera's confusing terminology: `executions' and
`iterations' are such similar terms that we keep tripping over them, which means
that everyone else probably trips over them even more. one fix might be to use
`processes' and `intra-process iterations' to make the difference clear.}
