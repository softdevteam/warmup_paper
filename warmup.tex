...
second). Second, we need to determine a sensible tolerance around the ideal APERF/MPERF ratio of 1.
...
on these two machines, 210 (0.004\%)  (spread over 6 process executions) failed
...
to deal with such aberrations, and 206 (98\%) of these in-process iterations are
detected as outliers, meaning that they do not affect changepoint analysis or
the statistics we produced in the 6 process executions they are part of.
The 4 in-process iterations which are not
...
coefficient of variance (standard deviation divided by the mean of measurements
up to that point) falls below a threshold. Using a threshold of 0.01 -- the more
...
state, Georges et al.'s heuristic also finds a steady state. However, it also
finds steady states for 78.1\% of the process executions we classify as no
