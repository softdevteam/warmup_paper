...
data before that changepoint in further searches. \laurie{i guess we want
to say something like ``although PELT assumes independence, by using a large
penalty $[$and we need to explain what `large means'!$]$, we are able to use
it on dependent data~\cite{antoch97effect}. See Section~\ref{sec:threats} for more details.}
...
the rates at which APERF and MPERF increment are opaque \edd{I don't think
opaque is the right word: you can see the values. Why don't we use Intel's
official terminology: "archtecturally defined"?} with only the APERF /
MPERF ratio being strictly defined~\cite{intel17pstate}, in
practise on our machines a fully busy core increments APERF proportional to the
CPU base frequency \edd{that's true of any machine. if it didn't increment
proportionally, then the ratio wouldn't work. The undefined aspect is "with
what proportion of the base frequency does it increment?". For our machines
it's a 1:1 mapping to the base frequency, but not necessarily on other
machines. It might be 1:7 or 1:3.3333,
or...} (e.g.~ \bencherfive's CPU increments APERF
at $3.6*10^9 Hz$).
...
least a 6\% difference in the APERF/MPERF ratio)\edd{Given what we know now, I
think the speedstep increments are irrelevant}\laurie{for the ``perfect''
upcoming version, maybe, but not for this version (which has to be much looser
in its tolerances)}\edd{Not sure what you mean. I'm referring to the fact that
it's kinda silly to think of the cores running at one of many discreet
speedstep values, when the effective clock speed is on a pretty much continuous scale (i.e. any
effective clock speed between the slowest and fastest values can be achieved by
switching between speedstep values in-iteration)}\laurie{yes, that's what the footnote
in the next sentence says. i don't think there's a perfect way of doing this,
so i'm happy enough with what we currently have, unless you have something
specific in mind to change it to?}\edd{I think we should say, we chose 3\% as
an acceptable tolerance and not mention the speedstep increments, since they
have no real bearing on the ratios we can see}\laurie{i don't agree. we (certainly i) used
them as a guideline for how big the ratio could be before we would
definitely be including actual speedsteps.}. Based on
this, we define the
...
iteration timings are clearly not. \rebecca{I'm not happy with this sentence as PELT 
doesn't make the independence assumption and there are many algorithms out there that 
do take into account the dependence - the problem is that they take too long 
computationally.  Sorry I should have explained why I changed it in my updates.} 
\laurie{ok. do you think you could draft a sentence that reflects this? i must admit i'm a bit confused!}
Fortunately, it has been shown that one can
safely use a changepoint approach that assumes independence by using a `larger'
penalty value~\cite{antoch97effect}. As this suggests, penalties are as much an
art as a science. A typical penalty is $4 \log n$ whereas we used $16 \log n$.
We manually inspected a large number of
...
produce are generally an over 
approximation. We ran a simulation study with similar dependencies to those observed in our 
segments \laurie{i think we need to give an intuition about what ``similar dependencies'' means. it 
also seems that two simulations were run, one with dependent data and one with independent data? i'm 
not sure how the comparison works. RK: Actually i've just looked back at the data to check what 
the dependence is in some of the files and I can't find any series that have dependence 
within segments (other than the bouncing ball ones), hence I don't think we even need to say 
this.  Unless you guys have a specific example or examples?}. Across 1,000
simulations 998 had larger, and only 2 had smaller, confidence intervals than
when the data were independent. Overall, despite
