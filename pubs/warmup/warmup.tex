...
into our benchmarks. Was it inside the main loop? If so, how?}. 

\laurie{this para needs checking} Second, we removed the fairly frequent use of
...
libraries, and the operating system kernel itself. We altered \laurie{all?
some?} benchmarks so that instead of intermediate printing, a checksum is
...
\item[Graal \#9dafd1dc5ff9] Oracle's next-gen Java VM. \laurie{do we actually run graal on its own, or simply as part of Truffle?}
...
reading. If the system fails to cool down, \krun terminates. \laurie{is there a
minimum sleep period?} \laurie{how does rebooting interact with temperature
stuff?}
