...
least a 6\% difference in the APERF/MPERF ratio). Based on this, we conservatively define the
safe APERF / MPERF ratio to be $1 \pm 0.03$. The checking tool then examined
...
the statistics we produced in 8 of the 10 process executions they are part of.
The 2 in-process iterations on \bencherfive are contained in two process executions
of \nbody on Graal, both at index 1000; both have an APERF/MPERF ratio 1.037.
Oddly, two process executions of \nbody on JRubyTruffle (which uses Graal for
compilation) on \bencherseven also have high APERF/MPERF ratios of 1.05 at
iteration 1000 (though these are identified as outliers). We have no explanation
for this, but the ratios are sufficiently below the ratio needed to indicate
turbo boost for us to discount them. The remaining 7 in-process iterations which are not
...
evenly spread over the (fairly noisy) process execution and that two of them led
