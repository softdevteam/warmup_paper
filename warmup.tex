...
\sarah{This is worth checking with Section 4.2 of Georges, which says (I believe) they decide they need $k$ iterations to take for the steady state, then they look through the run-sequence to find $k$ iterations for which the CoV is below a threshold. If they can find $k$ iterations below the threshold, these $k$ are taking to be the steady state and averaged to find the mean steady state performance time. So Georges do not look for an iteration number and say "we will take that iteration and all the subsequent ones to the end of the run-sequence".}
