...
\krun checks that it is running on a `tickless' kernel, which claims to reduce
`jitter' (variation) in time-sensitive workloads~\cite{tickless}. The default
Linux kernel interrupts each active logical CPU\footnote{Linux exposes
hyper-threads and multiple cores as logical CPUs.} 250 times (`ticks') a second
to decide whether to perform a context switch. On a tickless kernel, CPUs can
be put into `adaptive-tick' mode, under which a CPU only receives ticks if it
has more than one runnable process scheduled: if there is only one, context
switches cannot occur, so ticking is unnecessary. We compiled a kernel with
\texttt{CONFIG\_NO\_HZ\_FULL\_ALL} set, which places all CPUs bar one into
adaptive-tick mode.\footnote{It is not possible to have all CPUs in
adaptive-tick mode.} We ran a subset of our benchmarks on a default and the
tickless kernel. Although we observed small differences in variation, we were
unable to conclusively pin these effects to adaptive-ticks. Since tickless mode
does not appear have negative impact, we opted to leave it enabled.
...
In controlling confounding variables, our benchmarking environment deviates
substantially from the defaults. It could be argued that the setting we have
created is artificial as it is unlikely to match the user's environment.
Indeed, users should trial VMs on their own individual setups, rather than
relying solely on the findings presented in this paper.
