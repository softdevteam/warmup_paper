...
we defined in our experiment that changepoint segments less with means less than 0.001s
apart are equivalent. However, running in-process iterations for long periods
of time is also somewhat undesireable. As a rule of thumb, we
we suggest that the fastest VM should take a minimum of 0.1s, but ideally
around 1s, to a run a single-process iteration.
...
using a training set to devise heuristics, and then benchmarking the resulting system(s) on a separate
validation system.
