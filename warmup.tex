...
no steady state process executions are treated differently.
Consider a \vmbpair pair $P$ from the original results and a \vmbpair pair $P_s$
sampled from $P$. In all cases where one of $P$ or $P_s$ contains
no steady state process executions, the classifications comparison is made as normal (using the
confidence intervals). If $P$ and $P_s$ both contain at least one no steady state process execution,
then we do not count the steady iter (\# or s) and
steady perf (s) comparisons as either succeeding or failing (since no meaningful
comparison can be made). If one of $P$ or $P_s$ contains
at least one no steady state process execution, but the other contains
no steady state process executions, then the steady
