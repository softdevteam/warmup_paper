...
data before the changepoint in further searches.

There are various ways that one can determine when a changepoint has occurred, but the best fit
...
\subsection{How Long Did the Experiments Need To Run For?}
\label{how long}

\laurie{what i think we should try here is 3 graphs. first, we want a graph
showing the \%same curve (i.e. $x$ axis is 150...2000 (in-process iterations);
$y$ axis is ~70 to (exactly) 100\%. second, two jruby plots one at $n=1000$ and
one at $n=1050$ showing the different classifications (p13 in the PDF might be
good place to search for examples). we want the same pexec number in each case.}

We chose 2000 in-process iterations as our running length in part based on our
experience of VMs' internals. VMs have many counters which, when they exceed a
certain threshold, cause the VM to change behaviour (e.g.~(re)compile code,
change garbage collection strategy etc.) and we suspected that these counters
would be set at `common for human' values. Notably we suspected that some
counters would be set to 1000 (or, to make things look more `computer friendly',
perhaps to 1024) meaning that we might observe VM effects at in-process iterations
around this point. It thus made sense to run experiments substantially past that
point so that we would be able to see the long-term effects of any such
counters: we chose 2000 as our threshold.

However, no-one wants to run benchmarks for any longer than necessary to collect
reliable statistics and there are many attempts to dynamically adjust run
times to minimise the length of time benchmarks are run (see e.g.~\cite{chen16robust}
for an overview of such work in a non-JIT context). Assuming (as we must) that we ran experiments for `long
enough', a reasonable question is whether we needed to run benchmarks for as
long as we did. We thus performed a simple experiment on our data, running
shorter versions of each process execution through changepoint analysis and
seeing if this changed the overall classifications. For each process execution, we
performed changepoint analysis on the first $n$ in-process iterations where $150
\leq n \leq 1950$ in increments of 50 (shrinking the sliding window and last
segment sizes proportionally: i.e. at $n=1000$ the sliding window is 100 and
last segment 250) and simply counted how many process
executions received the same classifications as when $n=2000$. Intuitively one
would expect the classifications to gradually change as $n$ reduces, albeit not
monotonically (due to the very nature of changepoint analysis). Ideally one
would pick a value of $n$ that has a good trade-off between accuracy and running
time.

As Figure \laurie{XXX} (for \laurie{bencherX}) shows, there are significant
...
JRuby, many of whose process executions have a distinct performance spike at,
and generally just after, in-process iteration 1000, often causing a changepoint
segment to be created. With $n=2000$, changepoint analysis considers the slower
segment to be equivalent to the final segment (leading to many classifications
of warmup). At $n=1000$ the segment is not detected at all, thus generally
leading to a similar classification as at $n=2000$. At $n=1050$, however, the
segment is detected, and is thus the process execution's final segment; however
its mean and variance mean that the previous segment is generally considered
non-equivalent leading to a classification of no steady state (or, less often,
slowdown).

What this in mind, it seems reasonably clear to us that choosing a smaller
number of in-process iterations is largely dependent on the VM(s) in question
and one's confidence in their predictability. If the VM(s) do not have
...
course of action. It is therefore difficult to draw definitive conclusions from
our data: we would have got data with only 15\% different classifications with
650 in-process iterations, but with 21\% different classifications if we had
chosen 1050 in-process iterations!

\laurie{if we can work out the pexec situation we can also drop that in here}
...
running benchmarks for a longer period than suggested by previous methodologies.
Traditional VM experiments often run for 5 or 10 in-process iterations. Our data clearly shows
...
not always be feasible. Our analysis of our data in Section~\ref{how long}
does not make it easy to suggest what a good small number of in-process
iterations may be. However, a plausible approach would seem to be to mostly use
smaller numbers of in-process iterations (e.g.~500 in-process iterations) most
