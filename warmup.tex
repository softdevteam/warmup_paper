...
which were due to an interaction between the modifications we had made to
...
benchmark runs with the machine in a clean state. After each reboot, \krun is
invoked automatically by the system's init subsystem; \krun then pauses for 3
minutes to allow the system to fully initialise; calls \texttt{sync} (to
flush files to disk) followed by a 30 second wait; before finally running the
next process execution.

When \krun starts, it analyses its configuration file and creates a benchmark
running schedule which is written to a file. When \krun loads, it reads this
...
is exceeded, the processor is moved to a lower-power mode. Machines
may also have other safety limiters. To limit the extent to which this can
affect our benchmarks, we start each with the machine at a similar temperature.
Before its first process execution, \krun waits for 1 minute, before collecting
the values of all available temperature sensors. After each reboot, \krun waits
...
turn off network interfaces entirely, to prevent outside sources cause unbounded
interrupts to be sent to the processor and kernel.

In order to identify problems with the machine itself, \krun monitors the
system's \texttt{dmesg} buffer for any unknown changes (known `safe' changes
are whitelisted), informing the user if any occur. We implemented this after
