...
account.~\footnote{In future versions of this paper we will use a formal definition.}
...
process executions with cycles of length roughly 7 and 400 respectively.
...
\end{minipage}
\begin{minipage}{.65\textwidth}
\includegraphics[width=\textwidth]{examples/cycles2}
\end{minipage}
}
\caption{Examples of a benchmarks with cycles.\laurie{let's not show gcc stuff in the draft paper. it raises more questions than we can/want to answer.}\edd{I think we should keep it as it's probably the most interesting/insightful graph we have. Questions are fine: we don't plan to explain any of the behaviours we are seeing.}}
...
different pattern of randomness. Figure~\ref{fig:examples:late1} shows one such
example where the mean appears to step up and down in intervals of a few
hundred iterations.

\subsection{Unexpectedly Long Warmup}
\label{sub:long}

The common understanding of warmup behaviour usually assumes that for small
programs, such as our microbenchmarks, warmup is complete after a handful of
iterations. However, there are benchmarks where performance improves after many
hundred iterations. Figure~\ref{fig:examples:long} show an extreme example,
where warmup is not achieved until around iteration 1600.

\begin{figure}[h!]
\makebox[\textwidth][c]{
\begin{minipage}{.65\textwidth}
\includegraphics[width=\textwidth]{examples/late1}
\caption{Example of a benchmark with late phase changes.\laurie{this isn't
really a `late' phase change, as much as many phase changes (which, probably,
continue on changing for infinity)}\edd{Correct, but late phases changes none
the less. This was the best example I could find that couldn't be classified
as late warmup or late slow down. In a sense those two classifiers themselves
are late phase changes. I wonder if we should discard the `late phase change'
classifier altogether and re-file this graph under `variable mean` or
something.}}
...
Figure~\ref{fig:examples:inconsistent} shows two such cases: the top row shows
inconsistencies on the same machine, whereas the bottom row shows
...
\caption{Examples of benchmarks with inconsistencies between executions and machines.\laurie{we might want to be clearer about which are within and which are between machines?}}
