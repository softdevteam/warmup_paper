...
% I can't seem to get the table and figure side-by-side, with the captions
% intact, and with the whole thing floating. I've tried using a custom float and
% floatrow, but they both do weird stuff. The below typesetting works, but
% doesn't float and thus has to be placed manually. I can live with it.
%
\vspace{1em}
...
\vskip-1em
\captionof{figure}{An example showing that performance changes cannot always be
attributed to Just-In-Time compilation or garbage collection.}%
...
similar to \bencherthree).\footnote{\laurie{this is incorrect, right?} The large amount of CPU time our
