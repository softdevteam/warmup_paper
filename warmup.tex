...
\edd{XXX check all blurbs and captions, as the figures have changed}
...
\caption{\laurie{i don't think we want to introduce cycle counts for these early examples if we can avoid it: it's just one more thing to explain to readers} \laurie{also, for simplicities sake, let's use an example where the steady state is a single segment (like the LHS example above, but not like the RHS)} \laurie{it would also be good to use insets in both the LHS and RHS for consistency} Two examples of run-sequence plots from our experiment, augmented
with the results of changepoint analysis. The LHS plot
shows an example of `traditional' warmup (in this case \nbody on HHVM);
the RHS plot shows an example of slowdown (\richards on HotSpot). Changepoints
...
\sarah{bencher 3 is now bencher7 in the v0.8 data}
...
the final changepoint segment's mean). \laurie{this needs updating} For the latter two measures, we report the median time
...
\sarah{Curated plots are still v0.7 data -- or are they?}
